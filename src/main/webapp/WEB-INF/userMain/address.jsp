		...
		<ul style="margin-left:40px;" class="list">
			<c:forEach items="${addressList }" var="address">
				<li id="d${address.id }">
					...
					${address.provice } ${address.city } ${address.area }${address.address } 收货人：${address.username }  手机号：${address.phone }
				</li>
			</c:forEach>
			<li>
				...
				<a href="toaddAddress.do" class="btn-xiugai orange">增加</a>
			</li>
			<br>
		</ul>
	</form>
		...
		function shezhi() {
			...
			if (val == null) {
				...
				$.ajax({
					...
					url : 'setDefaultaddress.do', //发送数据的地址
					data : {
						id : val
					},
					dataType : "json",
			...
			$.ajax({
					...
					success : function(data) {
						...
						layer.alert("已删除该条地址！",{icon:1},function(index){
						    //do something
						    layer.close(index);
							window.location.href="userAddress.do";
						});
