...
<span <c:if test="${type=='1'}">class="active"</c:if>><a href="userMain.do?type=1" class="red">所有订单</a></span>
<span <c:if test="${type=='2'}">class="active"</c:if>><a href="userMain.do?type=2" class="red">待支付订单</a></span>
<span <c:if test="${type=='3'}">class="active"</c:if>><a href="userMain.do?type=3" class="red">待发货订单</a></span>
<span <c:if test="${type=='4'}">class="active"</c:if>><a href="userMain.do?type=4" class="red">待确认订单</a></span>
</div>
...
<form action="#" method="post" name="delform">
	  ...
	  <span class="good-status">交易状态</span> <span class="deal-action">交易操作</span></div>
		...
		<c:forEach items="${PageDate.data }" var="ordrList">
		 ...
		 <div class="order-detail">订单号：<span>${ordrList.id }</span>&nbsp;&nbsp;&nbsp;下单时间：<span><fmt:formatDate value="${ordrList.time }" pattern="yyyy-MM-dd"/> &nbsp;
					   </span></div>
         <!-- 列表详情区域开始 -->  
		...
		<!-- 左边商品区域开始 -->  
		  ...
		  <!-- 订单商品循环go -->
 		  ...
 		  <c:forEach items="${ordrList.itemList }" var="goods">
			  <div class="cos">
			  <span class="c1"><a href="goodsPage.do?id=${goods.goodsid }" class="dl"> <img src="${goods.propic }" width="70" height="70"></a>
			  <span class="dd"><a href="goodsPage.do?id=${goods.goodsid }" class="dd"> ${goods.goodsname }</a></span>
			 <span class="dd"></span>
			  </span>
 ...
 <div class="total-area"><p>￥${ordrList.total }</p><p>(货到付邮)</p> 
			...
			<p><a href="orderDetail.do?id=${ordrList.id }">订单详情</a></p>
			  </div>
			   <div class="act-area"> 
			   <c:if test="${ordrList.state==1 }">
			  <p><a href="javascript:cancelOrder(${ordrList.id })" >取消订单</a> </p>		
			  <p><a class="confirm" href="payorder.do?orderId=${ordrList.id }">前去支付</a> </p>
			  </c:if>	
	   ...
	   <!-- 订单循环结束 -->
	   	...
	   	function cancelOrder(id){
			...
			layer.confirm('确定要取消此条订单吗？', {title:'取消订单',icon: 3,shift:4,
				...
				location.href="cancelOrder.do?orderId="+id;
			});
		}
	   	</script>
	   	<style>
   ...
   <!-- 分页 --> 
       ...
       <div class="page"> 
        <div> 
        	<c:if test="${p!=1  }">
        		<a class="prev" href="userMain.do?type=${type }&p=${p-1 }">上一页</a>
        	</c:if>
        	<c:forEach begin="1" end="${PageDate.pagetotal }" varStatus="sta">
        		...
        		<c:if test="${p!=sta.count }">
        			<a class="num" href="userMain.do?type=${type }&p=${sta.count }">${sta.count }</a>
        		</c:if>
        	</c:forEach>
        	<c:if test="${p!=PageDate.pagetotal}">
        		<a class="next" href="userMain.do?type=${type }&p=${p+1 }">下一页</a>
        	</c:if>
